  ...
  return (
    <main className="page-wrap px-4 pb-8 pt-14">
      <section className="block-card overflow-hidden px-6 py-10 sm:px-10 sm:py-14">
        <p className="font-data text-xs font-bold uppercase tracking-[0.05em] text-[var(--on-surface-variant)] mb-3">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          >
            <p className="font-data text-3xl font-medium tracking-tight text-[var(--on-surface)] m-0">
              {value}
            </p>
            <p className="mt-2 font-data text-sm font-medium tracking-tight text-[var(--on-surface-variant)] m-0">
              {label}
            </p>
          </article>
        ))}
      ...
      <section className="block-card mt-8 p-6">
        ...
        <ul className="mt-4 space-y-2 pl-5 font-body text-sm text-[var(--on-surface-variant)]">
          ...
          <li>
            Configure your database in <code>src/db/schema.ts</code> and <code>drizzle.config.ts</code>.
          </li>
        </ul>
      </section>
    </main>
