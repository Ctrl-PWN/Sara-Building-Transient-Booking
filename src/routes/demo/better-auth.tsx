	...
	if (isPending) {
		return (
			<div className="flex items-center justify-center py-20">
				<div className="h-5 w-5 animate-spin rounded-full border border-[var(--outline)] border-t-transparent" />
			</div>
		);
	}
					...
					<div className="flex items-center gap-3">
						{session.user.image ? (
							<img
								...
								alt=""
								className="h-10 w-10 rounded-lg border border-[var(--outline)]"
							/>
						) : (
							<div className="h-10 w-10 rounded-lg border border-[var(--outline)] bg-[var(--surface-container)] flex items-center justify-center">
								<span className="font-body text-sm font-medium text-[var(--on-surface)]">
				...
				<form onSubmit={handleSubmit} className="grid gap-4">
					{isSignUp && (
						<div className="grid gap-2">
							<label
								htmlFor="name"
								className="font-body text-xs font-bold uppercase tracking-[0.05em] text-[var(--on-surface)]"
							>
								Name
							</label>
							<input
					...
					{error && (
						<div className="border border-[var(--error)] rounded-lg bg-[var(--error-container)] p-3">
							<p className="font-body text-sm text-[var(--on-error-container)]">
								{error}
							</p>
						</div>
					)}
					...
					>
						{loading ? (
							<span className="flex items-center justify-center gap-2">
								<span className="h-4 w-4 animate-spin rounded-full border border-[var(--on-primary)] border-t-transparent" />
