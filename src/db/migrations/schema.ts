import { pgTable, index, text, timestamp, foreignKey, serial, varchar, numeric, integer, unique, date, boolean, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const bookingPaymentStatus = pgEnum("booking_payment_status", ['CURRENT', 'OVERDUE', 'PAID_IN_FULL'])
export const bookingStatus = pgEnum("booking_status", ['RESERVED', 'CHECKED_IN', 'CHECKED_OUT', 'CANCELLED', 'EVICTED'])
export const ledgerTransactionCategory = pgEnum("ledger_transaction_category", ['ROOM_CHARGE', 'DEPOSIT', 'PAYMENT', 'REFUND', 'LATE_FEE', 'ADJUSTMENT'])
export const ledgerTransactionType = pgEnum("ledger_transaction_type", ['DEPOSIT', 'PAYMENT', 'REFUND', 'ADJUSTMENT'])
export const paymentMethod = pgEnum("payment_method", ['CASH', 'CARD', 'BANK_TRANSFER', 'GCASH'])
export const roomStatus = pgEnum("room_status", ['AVAILABLE', 'MAINTENANCE', 'OUT_OF_ORDER', 'OCCUPIED'])
...
export const bookings = pgTable("bookings", {
	...
	paymentStatus: bookingPaymentStatus("payment_status").default('CURRENT').notNull(),
